#!/usr/bin/python
# Simple python server to accept GET and Head request from local web browser and serving the Html web page 
...
class Server:
 ...
 def activate_server(self): 
     ...
     self.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM) #Create and return a new socket object
     try: # user provided in the __init__() port may be unavaivable
         print("Launching HTTP server on ", self.host, ":",self.port)
         self.socket.bind((self.host, self.port)) #Bind the socket to the address and port
 ...
 def _wait_for_connections(self):
     ...
     while True:
         ...
         self.socket.listen(100) # maximum number of queued connections
          
         conn, addr = self.socket.accept()#Return a client socket (with peer address information)
         ...
         if (request_method == 'GET') | (request_method == 'HEAD'):
             ...
             try:
                 ...
                 if (request_method == 'GET'):
                    response_content = b"<html><body><p>Error 404: File not found Lalit</p><p>Python HTTP server</p></body></html>"  
